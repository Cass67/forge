...
func TestDeriveTurnContractFromInput(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "implement named runtime work requires write action",
			input:       "implement runtime Turn Contract Kernel",
			wantIntent:  TurnIntentEditCode,
			wantActions: []ContractActionKind{ContractActionEdit},
		},
		{
			name:        "add support request requires write action",
			input:       "add support for JSON tool-call validation",
			wantIntent:  TurnIntentEditCode,
			wantActions: []ContractActionKind{ContractActionEdit},
		},
		{
			name:        "build named feature requires write action",
			input:       "build the turn contract kernel",
			wantIntent:  TurnIntentEditCode,
			wantActions: []ContractActionKind{ContractActionEdit},
		},
		{
			name:        "build feature request requires write action",
			input:       "build the feature",
			wantIntent:  TurnIntentEditCode,
			wantActions: []ContractActionKind{ContractActionEdit},
		},
		{
			name:        "change cli request requires write action",
			input:       "change the CLI to reject raw tool markup",
			wantIntent:  TurnIntentEditCode,
			wantActions: []ContractActionKind{ContractActionEdit},
		},
		{
			name:        "update named code requires write action",
			input:       "update the validator to catch embedded tool calls",
			wantIntent:  TurnIntentEditCode,
			wantActions: []ContractActionKind{ContractActionEdit},
		},
		{
			name:        "modify named code requires write action",
			input:       "modify the prompt contract derivation",
			wantIntent:  TurnIntentEditCode,
			wantActions: []ContractActionKind{ContractActionEdit},
		},
		{
			name:        "modify cli request requires write action",
			input:       "modify the CLI",
			wantIntent:  TurnIntentEditCode,
			wantActions: []ContractActionKind{ContractActionEdit},
		},
		{
		...
		{
			name:        "polite fix question requires write action",
			input:       "Can you fix this?",
			wantIntent:  TurnIntentEditCode,
			wantActions: []ContractActionKind{ContractActionEdit},
		},
		{
			name:        "polite update question requires write action",
			input:       "Can you update the validator?",
			wantIntent:  TurnIntentEditCode,
			wantActions: []ContractActionKind{ContractActionEdit},
		},
		{
			name:       "please explain build stays answer only",
			input:      "please explain how to build the project",
			wantIntent: TurnIntentAnswerOnly,
		},
		{
			name:       "explain update flow stays answer only",
			input:      "explain the update validator flow",
			wantIntent: TurnIntentAnswerOnly,
		},
		{
			name:        "review build logs requires inspection only",
			input:       "review build logs",
			wantIntent:  TurnIntentInspect,
			wantActions: []ContractActionKind{ContractActionRead},
		},
		{
			name:       "build question is answer only",
			input:      "how do I build the turn contract kernel?",
			wantIntent: TurnIntentAnswerOnly,
		},
		{
			name:       "add support question is answer only",
			input:      "Can you explain how to add support for plugins?",
			wantIntent: TurnIntentAnswerOnly,
		},
		{
			name:       "read only update explanation is answer only",
			input:      "explain how to update the validator",
			wantIntent: TurnIntentAnswerOnly,
		},
		{
