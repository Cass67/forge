...
func TestSessionDurableSinkOwnsPersistedThreadIdentity(t *testing.T) {
	...
	if len(items) != 4 {
		t.Fatalf("persisted items = %#v, want user, assistant, tool result, and failure", items)
	}
	...
	if items[0].ID != "durable-thread-1-000001" || items[3].ID != "durable-thread-1-000004" {
		t.Fatalf("persisted IDs start/end = %q, %q; want durable-thread IDs", items[0].ID, items[3].ID)
	}
