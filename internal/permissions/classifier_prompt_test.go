...
func TestBuildClassifierPromptRedactsSecrets(t *testing.T) {
	...
	if got := strings.Count(prompt, "<REDACTED:github-pat>"); got != 4 {
		t.Fatalf("redaction count = %d, want 4", got)
	}
	if req.Action.Summary != "use token "+secret || req.Action.Detail != "TOKEN="+secret || req.Transcript != "user pasted "+secret || req.Rules[0].Pattern != "token="+secret {
