...
func TestParseDelegateOutcomeStructuredEnvelopeCarryContextKeepsRichMessageAndThinArtifact(t *testing.T) {
	outcome := parseDelegateOutcomeForRole("scout", `{"status":"complete","message":"Repository purpose: Forge is a Go-based terminal-first local coding agent. Evidence-backed findings: High severity - Go 1.25.0 may constrain contributor setup.","artifact_kind":"repo_review","artifact":["README.md","go.mod","BUILD.md"],"next_role":"architect","next_task":"Synthesize these findings."}`)
	got := outcome.CarryContextText()
	for _, want := range []string{
		"Repository purpose: Forge is a Go-based terminal-first local coding agent.",
		"High severity - Go 1.25.0 may constrain contributor setup.",
		`["README.md","go.mod","BUILD.md"]`,
	} {
		if !strings.Contains(got, want) {
			t.Fatalf("carry context missing %q: %q", want, got)
		}
	}
}
...
func TestParseDelegateOutcomeForRoleUsesArchitectThemeSummaryWhenMessageIsMeta(t *testing.T) {
	outcome := parseDelegateOutcomeForRole("architect", `{"status":"complete","message":"Synthesized scout evidence into higher-level assessment themes strictly grounded in cited files, without introducing new claims.","artifact_kind":"assessment_themes","artifact":{"scope":"repo-review","themes":[{"title":"CLI-first orchestration with broad command surface","summary":"The repository centers on a single CLI entrypoint that dispatches many commands and initiates interactive/TUI session flows."},{"title":"Session lifecycle complexity concentrated in entrypoint flow","summary":"A substantial loop in main coordinates live rendering/events, feedback capture, post-session fix handling, gate reinitialization, and restart behavior."}]}}`)
	got := outcome.DisplayText()
	for _, want := range []string{
		"CLI-first orchestration with broad command surface",
		"Session lifecycle complexity concentrated in entrypoint flow",
	} {
		if !strings.Contains(got, want) {
			t.Fatalf("display text missing %q: %q", want, got)
		}
	}
	if strings.Contains(got, "Synthesized scout evidence into higher-level assessment themes") {
		t.Fatalf("display text should prefer richer architect artifact summary over meta message: %q", got)
	}
}

func TestParseDelegateOutcomeForRoleUsesArchitectPriorityThemeSummaryWhenMessageIsMeta(t *testing.T) {
	outcome := parseDelegateOutcomeForRole("architect", `{"status":"complete","message":"Synthesized repo-level recommendations from scout evidence with focus on maintenance risk, documentation drift, dependency/toolchain constraints, and entrypoint complexity.","artifact_kind":"repo_review_recommendations","artifact":{"priorities":[{"rank":1,"theme":"Toolchain and dependency constraints","findings":[{"evidence":"go.mod:3","risk":"Go 1.25.0 requirement may reduce contributor/CI buildability without pinned environments.","recommendations":["Confirm whether Go 1.25 features are required.","If not strictly required, lower minimum Go version to a broadly available baseline and enforce via CI matrix."]}]},{"rank":2,"theme":"Entrypoint complexity and maintainability","findings":[{"evidence":"cmd/forge/main.go:36-90, 104-199, 219-257 and repeated os.Exit usage","risk":"Centralized dispatcher mixes interactive startup, orchestration, repair loops, and admin commands, creating high coupling and regression risk.","recommendations":["Refactor to thin main: move command handlers into domain packages (e.g., internal/cmd/*)."]}]},{"rank":3,"theme":"Documentation drift and user path clarity","findings":[{"evidence":"README.md:246-255","risk":"Absolute filesystem paths are environment-specific and break for other users.","recommendations":["Replace absolute paths with repository-relative links."]}]}]}}`)
	got := outcome.DisplayText()
	for _, want := range []string{
		"Toolchain and dependency constraints",
		"Entrypoint complexity and maintainability",
		"Documentation drift and user path clarity",
		"Confirm whether Go 1.25 features are required",
	} {
		if !strings.Contains(got, want) {
			t.Fatalf("display text missing %q: %q", want, got)
		}
	}
	if strings.Contains(got, "Synthesized repo-level recommendations from scout evidence") {
		t.Fatalf("display text should prefer richer architect priority summary over meta message: %q", got)
	}
}

func TestParseDelegateOutcomeForRoleUsesArchitectPrioritizedFindingsAndNextActionsWhenMessageIsMeta(t *testing.T) {
	outcome := parseDelegateOutcomeForRole("architect", `{"status":"complete","message":"Synthesized scout evidence into prioritized findings, risks, and concrete next actions for repo audit.","artifact_kind":"repo_review_synthesis","artifact":{"prioritized_findings":[{"priority":1,"title":"Core agent orchestration complexity is actively increasing","impact":"High regression risk in central dispatch path; harder reasoning/debugging as features accrete."},{"priority":2,"title":"Sub-agent/tool-call parsing appears brittle","impact":"Protocol-boundary fragility can cause intermittent failures and recovery-only behavior."}],"next_actions":[{"order":1,"action":"Stabilize protocol boundaries for sub-agent/tool-call handling","details":["Define strict internal message contract","Normalize/validate at one ingress layer"],"expected_outcome":"Reduced flakiness and clearer failure modes."},{"order":2,"action":"Modularize agent dispatch responsibilities","details":["Separate retry policy, chaining policy, and task parsing into focused units"],"expected_outcome":"Lower regression risk and improved maintainability."}]}}`)
	got := outcome.DisplayText()
	for _, want := range []string{
		"Core agent orchestration complexity is actively increasing",
		"Sub-agent/tool-call parsing appears brittle",
		"Stabilize protocol boundaries for sub-agent/tool-call handling",
		"Reduced flakiness and clearer failure modes",
	} {
		if !strings.Contains(got, want) {
			t.Fatalf("display text missing %q: %q", want, got)
		}
	}
	if strings.Contains(got, "Synthesized scout evidence into prioritized findings, risks, and concrete next actions for repo audit.") {
		t.Fatalf("display text should prefer prioritized findings/action summary over meta message: %q", got)
	}
}

func TestParseDelegateOutcomeForRoleUsesArchitectRiskPrioritizationWhenMessageIsMeta(t *testing.T) {
	outcome := parseDelegateOutcomeForRole("architect", `{"status":"complete","message":"Synthesized evidence-backed findings into prioritized risks and actionable recommendations with phased execution.","artifact_kind":"risk_prioritization_and_recommendations","artifact":{"executive_summary":"Primary risk is complexity concentration across CLI control flow, dual execution modes, and provider/config matrix.","risk_prioritization":[{"priority":1,"title":"Inconsistent failure semantics across runtime paths","severity":"high","impact":"Unclear user outcomes (silent no-op vs handled error vs abrupt termination), reduced reliability in interactive workflows.","recommendations":["Define error taxonomy (user/config/provider/internal) and apply consistently.","Replace operational panics with typed errors and top-level handling."]},{"priority":2,"title":"Dual-mode compatibility burden (chat + legacy make pipeline)","severity":"high","impact":"Expanded regression surface, slower refactors, ongoing compatibility overhead.","recommendations":["Set lifecycle status for legacy mode (active/maintenance/deprecation)."]}]}}`)
	got := outcome.DisplayText()
	for _, want := range []string{
		"Inconsistent failure semantics across runtime paths",
		"Dual-mode compatibility burden (chat + legacy make pipeline)",
		"Define error taxonomy (user/config/provider/internal) and apply consistently",
	} {
		if !strings.Contains(got, want) {
			t.Fatalf("display text missing %q: %q", want, got)
		}
	}
	if strings.Contains(got, "Synthesized evidence-backed findings into prioritized risks and actionable recommendations with phased execution.") {
		t.Fatalf("display text should prefer risk prioritization summary over meta message: %q", got)
	}
}
