...
func TestRunChatTurnCompletesComplexVisiblePreviewTurn(t *testing.T) {
	...
	reg := tools.NewRegistry()
	previewRuntime, _, _, _ := registerTools(reg, workDir, cfg, reactruntime.NewSession(), approve, nil, nil)
...
func TestRegisterToolsIncludesPreviewLifecycleTools(t *testing.T) {
	reg := tools.NewRegistry()
	cfg := &config.Config{}
	_, _, _, _ = registerTools(reg, t.TempDir(), cfg, reactruntime.NewSession(), agent.YoloApproval(), nil, nil)
...
func TestRegisterReactDelegationToolsAddsSpawnWaitAndOutputAlias(t *testing.T) {
	...
	workDir := t.TempDir()
	approve := agent.YoloApproval()
	_, _, _, _ = registerTools(reg, workDir, cfg, reactruntime.NewSession(), approve, nil, nil)
	baseReg := reg.Filter(nil)
...
func TestRegisterToolsAddsGitMergeStatus(t *testing.T) {
	...
	_, _, _, _ = registerTools(reg, workDir, cfg, reactruntime.NewSession(), agent.YoloApproval(), nil, nil)
...
func TestRegisterToolsDoesNotBlockOnMCPConnect(t *testing.T) {
	...
	reg := tools.NewRegistry()
	start := time.Now()
	_, mcpManager, execManager, _ := registerTools(reg, t.TempDir(), cfg, reactruntime.NewSession(), agent.YoloApproval(), nil, nil)
...
func TestRegisterDelegationToolsStripsGitMutationFromChildren(t *testing.T) {
	...
	childReg := childRegistryForRole(reg)
...
func TestNewChildAgentRegistryCreatesToolsWithCorrectWorkDir(t *testing.T) {
	...
	_, _, _, _ = registerTools(parentReg, parentDir, cfg, reactruntime.NewSession(), approve, nil, nil)
