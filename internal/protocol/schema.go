...
func GenerateProtocolSchema() JSONSchema {
	return JSONSchema{
		...
		"properties": JSONSchema{
			...
			"id":        stringSchema(),
			"kind":      JSONSchema{"enum": []string{string(ItemSessionMeta), string(ItemTurnContext), string(ItemUserMessage), string(ItemAssistantMessage), string(ItemToolCall), string(ItemToolResult), string(ItemRetry), string(ItemFailure), string(ItemStats), string(ItemCompaction), string(ItemTurnComplete)}, "type": "string"},
			...
			"tool_result":   objectSchema(map[string]any{"diff": stringSchema(), "is_error": JSONSchema{"type": "boolean"}, "original_bytes": JSONSchema{"type": "integer"}, "text": stringSchema(), "tool_call_id": stringSchema(), "tool_name": stringSchema(), "truncated": JSONSchema{"type": "boolean"}}),
