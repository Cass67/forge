...
- This file holds the repo-guard security lockdown rules and takes precedence.
- The project's own agent security guidance lives in `AGENT_SECURITY_NOTES.md`:
  blocked credential paths, secret redaction, and scope limits on searching
  `$HOME`. Apply the rules in this file first and treat that file as
  supplemental.
