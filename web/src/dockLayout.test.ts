...
test("a dock cannot be dragged shut or over the chat column", () => {
  ...
  // The chat keeps a floor, but a dock is allowed most of the shell: a
  // terminal you want to work in should not stop at a third of the window.
  expect(wide.right + wide.left).toBeLessThanOrEqual(0.9);
  expect(wide.right).toBeGreaterThan(0.6);
});

test("the other dock's width limits this one", () => {
  const fat = { left: 0.5, right: 0.1 };
  expect(clampDock(fat, "right", 0.5).right).toBeCloseTo(0.4);
});
...
test("stored widths are kept, and junk falls back to the defaults", () => {
  ...
  // Two docks asking for everything still leave the chat its floor.
